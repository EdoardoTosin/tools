...
################################################################################
# Caddy Setup Script for Raspberry Pi
#
# This script installs Caddy web server on a Raspberry Pi, sets up a basic
...
Options:
  -w, --www-path       Specify the WWW root path (default: /var/www)
  -r, --web-root       Specify the web root directory (default: localhost)
...
WWW_PATH="${WWW_PATH:-/var/www}"                 # Default web root path
WEB_ROOT="${WEB_ROOT:-localhost}"                # Default web root directory

# Step 1: Install Caddy
color_echo GREEN "Installing Caddy..."
sudo apt install -y debian-keyring debian-archive-keyring apt-transport-https curl || error_exit "Failed to install Caddy."

# Import Caddy GPG key
if [ ! -f "/usr/share/keyrings/caddy-stable-archive-keyring.gpg" ]; then
    # If the file doesn't exist, import the Caddy GPG key
    if ! curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | sudo gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg; then
        error_exit "Failed to import Caddy GPG key."
    fi
else
    # If the file already exists, echo a message
    color_echo YELLOW "The file '/usr/share/keyrings/caddy-stable-archive-keyring.gpg' already exists."
fi

# Add Caddy repository
curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | sudo tee /etc/apt/sources.list.d/caddy-stable.list || error_exit "Failed to add Caddy repository."
sudo apt update || error_exit "Failed to update package lists."
sudo apt install -y caddy || error_exit "Failed to install Caddy."

# Step 2: Create the necessary directories
color_echo GREEN "Creating $WWW_PATH and $WWW_PATH/$WEB_ROOT directories..."
sudo mkdir -p "$WWW_PATH" || error_exit "Failed to create $WWW_PATH directory."
sudo mkdir -p "$WWW_PATH/$WEB_ROOT" || error_exit "Failed to create $WWW_PATH/$WEB_ROOT directory."

# Check if index.html exists before writing to it
if [ ! -f "$WWW_PATH/$WEB_ROOT/index.html" ]; then
    color_echo GREEN "Writing HTML content to $WWW_PATH/$WEB_ROOT/index.html..."
    # Write HTML content to index.html
    cat <<EOF | sudo tee "$WWW_PATH/$WEB_ROOT/index.html" > /dev/null
<!DOCTYPE html>
<html>
    <head>
        ...
        <link rel="icon" href="https://caddyserver.com/resources/images/favicon.png" />
        <style>
            * {
        ...
        <div class="stack">
            ...
            <div class="paper" id="paper3">
                ...
                <ol>
                    ...
                    <li>Upload your site's files to <code>$WWW_PATH/$WEB_ROOT</code>.</li>
                    <li>
                ...
                <p>
                    Remember, Caddy can do a lot more than serve static files. It's also a powerful reverse proxy and application platform. You can use the Caddyfile to enable any other features you need. Or you could use Caddy's API to
                    configure it programmatically.
                </p>
                <p>Everything you need to know is either in the <a href="https://caddyserver.com/docs/">📖 Caddy documentation</a> or the manual for your OS/platform. Have fun!</p>
                ...
                <ul>
                    ...
                    <li>Are your site's files readable by the caddy user and group? <code>ls -la $WWW_PATH/$WEB_ROOT</code></li>
                ...
                <p>
                    <b class="warn">WAIT!</b> Before trying again, switch to <a href="https://letsencrypt.org/docs/staging-environment/">Let's Encrypt's staging environment</a> to avoid being accidentally
                    <a href="https://letsencrypt.org/docs/rate-limits/">rate limited</a>. Once you get everything else working, it's safe to switch back.
                </p>
                <p>
                    Depending on your DNS provider, it may take a while for the DNS records to propagate. Even when everything is configured properly, automated challenges to obtain TLS certificates usually take several seconds, but may
                    take up to several minutes or hours.
                </p>

                <p>
                    If you still need help, we have a <a href="https://caddy.community/">great community</a>! First <a href="https://caddy.community/search">try a search</a>, and if your question is original, go ahead and ask it! Remember
                    to pay it forward and help others too. 😁
                </p>
                <p>
                    Visit Caddy on:
...
EOF
    ...
    color_echo YELLOW "$WWW_PATH/$WEB_ROOT/index.html already exists. Skipping writing HTML content."
fi

# Step 3: Modify /etc/caddy/Caddyfile
color_echo GREEN "Modifying /etc/caddy/Caddyfile..."
sudo sed -i "s|^\troot \* /usr/share/caddy$|\t#root * /usr/share/caddy\n\troot * $WWW_PATH/$WEB_ROOT\n\tencode gzip|" /etc/caddy/Caddyfile || error_exit "Failed to modify Caddyfile."

# Step 4: Enable and start the Caddy API service
color_echo GREEN "Enable and Start Caddy API service..."
sudo systemctl enable --now caddy-api || error_exit "Failed to enable and start Caddy API service."

# Provide setup completion message
color_echo GREEN "Setup completed successfully!"
color_echo GREEN "Caddy is installed and configured to serve content from $WWW_PATH/$WEB_ROOT."
color_echo GREEN "Caddy has been configured to run at boot."
echo "Reboot your Raspberry Pi to test the setup."
